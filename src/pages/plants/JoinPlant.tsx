...
import { QrCode, ScanLine, X, Loader2 } from "lucide-react";
import { Html5Qrcode } from "html5-qrcode";
import { joinPlant } from "../../api/plants";
...
export default function JoinPlant() {
  ...
  const handleSubmit = async (e?: FormEvent) => {
    ...
    if (!joinKey.trim()) {
      setError("Please enter a sharing token.");
      return;
    }
    ...
    try {
      const plant = await joinPlant(joinKey.trim());
      localStorage.removeItem("cached_plants");
      setSuccess(`Joined plant "${plant.nickname}" successfully.`);
      setSubmitting(false);
      setTimeout(() => {
      ...
      if (err instanceof ApiError) {
        ...
        setError("Failed to join plant. Please try again.");
      }
      setSubmitting(false);
    }
  };
  ...
  return (
    <PageContainer>
      ...
      <PageTitle
        title="Join a Plant"
        subtitle="Scan the QR code from another user's plant sharing page or enter the sharing token manually."
      />
      ...
      <div className="bg-beige-50 rounded-2xl border border-beige-300 p-8 shadow-sm mb-6">
        ...
        <p className="text-forest-500 text-sm mb-6">
          Use your camera to scan the QR code from the plant sharing page.
        </p>
      ...
      <div className="bg-beige-50 rounded-2xl border border-beige-300 p-8 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            >
              Enter the sharing token from the plant owner
            </label>
